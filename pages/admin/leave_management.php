...
$stmt = $pdo->prepare("
    SELECT l.*, e.first_name, e.last_name, e.employee_id, e.email as emp_email, e.phone as emp_phone, d.name as department_name
    FROM leaves l
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-surface-muted border-b border-border-subtle">
                        ...
                        <th class="text-left px-6 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">Applied</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($leaves)): ?>
                    ...
                    <tr class="hover:bg-surface-muted transition-colors border-b border-border-subtle cursor-pointer" 
                        ...
                        <td class="px-6 py-4 text-body-sm text-secondary"><?= date('M d, Y', strtotime($l['created_at'])) ?></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
...
<!-- Leave Details Modal -->
<div id="leave-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40" onclick="closeLeaveModal(event)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden" onclick="event.stopPropagation()">
        <div class="p-6 border-b border-border-subtle flex items-center justify-between">
            ...
            <button onclick="closeLeaveModal()" class="w-8 h-8 rounded-lg border border-border-subtle flex items-center justify-center text-secondary hover:bg-surface-muted">
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="flex items-center gap-4 pb-4 border-b border-border-subtle">
                <div id="modal-avatar" class="w-14 h-14 rounded-full bg-primary-container flex items-center justify-center text-lg font-bold text-on-primary-container"></div>
                <div>
...
<!-- Confirmation Modal -->
<div id="confirm-modal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/50" onclick="closeConfirmModal(event)">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden" onclick="event.stopPropagation()">
        <div class="p-6 text-center">
            ...
            <div class="flex gap-3">
                <button onclick="closeConfirmModal()" class="flex-1 px-4 py-2.5 border border-border-subtle rounded-lg text-body-sm font-medium hover:bg-surface-muted">Cancel</button>
                <form id="confirm-form" method="POST" class="flex-1">
                    ...
                    <button type="submit" id="confirm-btn" class="w-full px-4 py-2.5 rounded-lg text-body-sm font-bold text-white"></button>
                </form>
            </div>
        </div>
    </div>
