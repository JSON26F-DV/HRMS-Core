    ...
    <div class="flex justify-between items-end">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-on-surface flex items-center gap-2"><img
                    src="<?= BASE_URL ?>/public/emojis/Title%20emojis/audit.png" class="w-8 h-8" alt=""> Audit Logs</h2>
            <p class="text-text-body font-body-md">Track all system activities and changes.</p>
        </div>
        <button onclick="openDeleteModal()" id="delete-btn"
            class="hidden items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg font-bold text-sm">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-surface-muted border-b border-border-subtle">
                            <th
                                class="text-left px-4 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold w-12">
                                <input type="checkbox" id="select-all" onchange="toggleAll(this)"
                                    class="w-5 h-5 rounded border-border-subtle cursor-pointer">
                            </th>
                            <th
                                class="text-left px-4 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">
                                Timestamp</th>
                            <th
                                class="text-left px-4 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">
                                User</th>
                    ...
                    <tbody>
                        <?php if (empty($logs)): ?>
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-secondary">No audit logs found.</td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr class="hover:bg-surface-muted transition-colors border-b border-border-subtle">
                                    <td class="px-4 py-4">
                                        <input type="checkbox" name="selected_logs[]" value="<?= $log['id'] ?>"
                                            class="log-checkbox w-5 h-5 rounded border-border-subtle cursor-pointer"
                                    ...
                                    <td class="px-4 py-4">
                                        <span
                                            class="px-2.5 py-1 bg-surface-muted rounded-lg text-xs font-bold"><?= h($log['action']) ?></span>
                                    </td>
                                    <td class="px-4 py-4 text-body-sm"><?= h($log['entity_type'] ?? '--') ?>
                                        #<?= h($log['entity_id'] ?? '--') ?></td>
                                    <td class="px-4 py-4 text-body-sm text-secondary max-w-xs truncate"
                                        title="<?= h($log['details'] ?? '') ?>"><?= h($log['details'] ?? '') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
...
<div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40"
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden" onclick="event.stopPropagation()">
        <div class="p-6 text-center">
            ...
            <p class="text-body-sm text-secondary mb-6">Are you sure you want to delete <span id="delete-count">0</span>
                selected log(s)? This action cannot be undone.</p>
            <div class="flex gap-3">
                ...
                <button onclick="submitDelete()"
                    class="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg text-body-sm font-bold">Delete</button>
            </div>
        </div>
    </div>
</div>
