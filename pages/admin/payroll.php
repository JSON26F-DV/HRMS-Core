    ...
    if (isset($_POST['generate'])) {
        ...
        if (empty($empIds)) {
            ...
            foreach ($empIds as $eid) {
                ...
                $attSummary = implode(', ', array_map(function($k,$v){return "$k: $v";}, array_keys($statusCounts), $statusCounts));
                $deductions = round($gross * 0.1, 2);
                $net = $gross - $deductions;

                $stmt = $pdo->prepare("INSERT INTO payroll (employee_id, period_start, period_end, gross_pay, deductions, net_pay, days_worked, total_hours, attendance_summary, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')");
                $stmt->execute([$eid, $start, $end, $gross, $deductions, $net, $days, $totalHrs, $attSummary]);
...
if (isset($_GET['print'])) {
    ...
    ?><!DOCTYPE html><html><head><meta charset="utf-8"><title>Payslip</title><style>
        ...
        .summary div span{display:block;font-size:12px;color:#888;margin-bottom:4px}
...
function calcDayPay($status, $rate, $minutes_late = 0) {
    ...
    if ($status === 'half_day') return round($rate / 2, 2);
    if ($status === 'late' && $minutes_late > 0) {
...
$unpaidParams = [];
if ($uStart) { $unpaidWhere[] = "a.date >= ?"; $unpaidParams[] = $uStart; }
if ($uEnd) { $unpaidWhere[] = "a.date <= ?"; $unpaidParams[] = $uEnd; }
...
<div class="max-w-7xl mx-auto space-y-8">
    <div class="flex justify-between items-end">
        <div><h2 class="font-headline-lg text-headline-lg text-on-surface flex items-center gap-2"><img src="<?= BASE_URL ?>/public/emojis/Title%20emojis/payroll.png" class="w-8 h-8" alt=""> Payroll Management</h2><p class="text-text-body font-body-md">Generate payroll from attendance records.</p></div>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-2xl shadow-sm border border-border-subtle overflow-hidden">
        <div class="p-5 border-b border-border-subtle flex items-center justify-between flex-wrap gap-3">
            ...
            <form method="GET" class="flex items-center gap-2">
                <input type="date" name="u_start" value="<?= h($uStart) ?>" class="h-9 px-3 bg-surface-muted border border-border-subtle rounded-lg text-sm" placeholder="From">
                <input type="date" name="u_end" value="<?= h($uEnd) ?>" class="h-9 px-3 bg-surface-muted border border-border-subtle rounded-lg text-sm" placeholder="To">
                <button type="submit" class="h-9 px-4 bg-primary-container text-white font-bold rounded-lg text-sm hover:brightness-95">Filter</button>
                <?php if ($uStart || $uEnd): ?><a href="?" class="h-9 px-4 border border-border-subtle rounded-lg text-sm text-secondary flex items-center hover:bg-surface-muted">Clear</a><?php endif; ?>
            </form>
        </div>
        ...
        <form method="POST" id="payroll-form">
            <input type="hidden" name="period_start" value="<?= h($uStart ?: date('Y-m-d', strtotime('first day of this month'))) ?>">
            <input type="hidden" name="period_end" value="<?= h($uEnd ?: date('Y-m-d', strtotime('last day of this month'))) ?>">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="bg-surface-muted border-b border-border-subtle">
                            <th class="text-left px-4 py-3 text-label-sm text-secondary uppercase tracking-widest font-bold w-10"><input type="checkbox" id="select-all" onchange="toggleAll()"></th>
                            <th class="text-left px-4 py-3 text-label-sm text-secondary uppercase tracking-widest font-bold">Employee</th>
                            <th class="text-left px-4 py-3 text-label-sm text-secondary uppercase tracking-widest font-bold">Date</th>
                            <th class="text-left px-4 py-3 text-label-sm text-secondary uppercase tracking-widest font-bold">Status</th>
                            <th class="text-left px-4 py-3 text-label-sm text-secondary uppercase tracking-widest font-bold">In</th>
                    ...
                    <tbody>
                        <?php $empIdx = 0; foreach ($unpaidByEmp as $eid => $eg): ?>
                        <?php $totalP = 0; $totalD = 0; $totalLate = 0; ?>
                        <tr class="bg-surface-container-low border-b border-border-subtle">
                            <td class="px-4 py-2" colspan="9">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" class="emp-group" data-emp="<?= $eid ?>" onchange="toggleGroup(<?= $eid ?>)">
                                    <span class="font-bold text-body-sm"><?= h($eg['name']) ?> (<?= h($eg['code']) ?>)</span>
                                    <span class="text-xs text-secondary">— Rate: ₱<?= number_format($eg['rate'] ?? 0, 2) ?>/day</span>
                                </label>
                            </td>
                        </tr>
                        <?php foreach ($eg['rows'] as $r):
                            ...
                            $minutesLate = (int)($r['minutes_late'] ?? 0);
                            $dayPay = calcDayPay($r['status'], $rate, $minutesLate);
                            $totalP += $dayPay; if ($dayPay > 0) $totalD++;
                            $totalLate += $minutesLate;
                        ?>
                        <tr class="hover:bg-surface-muted transition-colors border-b border-border-subtle">
                            <td class="px-4 py-2"><input type="checkbox" name="employee_ids[]" value="<?= $eid ?>" class="emp-cb emp-<?= $eid ?>"></td>
                            <td class="px-4 py-2 text-body-sm"><?= h($r['date']) ?></td>
                            <td class="px-4 py-2">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase <?= $r['status']==='present'?'bg-green-100 text-green-700':($r['status']==='late'?'bg-yellow-100 text-yellow-700':($r['status']==='half_day'?'bg-blue-100 text-blue-700':'bg-red-100 text-red-700')) ?>"><?= ucfirst($r['status']) ?></span>
                            </td>
                        ...
                        <tr class="border-b border-border-subtle bg-surface-muted/50">
                            ...
                            <td class="px-4 py-2 font-mono font-bold text-body-sm"><?= $totalLate ? $totalLate . 'm' : '—' ?></td>
                            <td class="px-4 py-2"></td>
                            <td class="px-4 py-2 font-mono font-bold text-body-sm">₱<?= number_format($totalP, 2) ?></td>
                        </tr>
            ...
            <div class="p-4 border-t border-border-subtle flex justify-between items-center">
                ...
                <button type="submit" name="generate" class="h-10 px-6 bg-primary-container text-white font-bold rounded-lg text-sm hover:brightness-95">Generate Payroll for Selected</button>
            </div>
        </form>
        <?php endif; ?>
    </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-surface-muted border-b border-border-subtle">
                        ...
                        <th class="text-left px-6 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">Attendance</th>
                        <th class="text-right px-6 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">Days</th>
                        <th class="text-right px-6 py-4 text-label-sm text-secondary uppercase tracking-widest font-bold">Hours</th>
                ...
                <tbody>
                    <?php if (empty($prows)): ?>
                    <tr><td colspan="10" class="px-6 py-12 text-center text-secondary">No payroll records yet.</td></tr>
                    <?php else: ?>
                    ...
                    <tr class="hover:bg-surface-muted transition-colors border-b border-border-subtle">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <form method="POST" class="inline"><input type="hidden" name="id" value="<?= $p['id'] ?>"><button type="submit" name="delete" onclick="return confirm('Delete? This will revert attendance to unpaid.')" class="w-8 h-8 rounded-lg border border-border-subtle flex items-center justify-center text-secondary hover:bg-red-100 transition-all" title="Delete">✕</button></form>
